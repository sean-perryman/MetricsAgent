...
HttpResponse httpPostUtf8(const std::wstring& url, const std::wstring& headers, const std::string& bodyUtf8, int timeoutSeconds) {
  ...
  if (!WinHttpCrackUrl(url.c_str(), 0, 0, &uc)) {
    resp.error = L"WinHttpCrackUrl failed: " + lastErrorToString();
    return resp;
  }
  ...
  HINTERNET hSession = WinHttpOpen(L"MetricsAgent/1.0", WINHTTP_ACCESS_TYPE_DEFAULT_PROXY,
  ...
  if (!hRequest) {
    resp.error = L"WinHttpOpenRequest failed: " + lastErrorToString();
    WinHttpCloseHandle(hConnect); WinHttpCloseHandle(hSession); return resp;
  }
  ...
  if (!ok) {
    resp.error = L"WinHttpSendRequest failed: " + lastErrorToString();
  ...
  if (!ok) {
    resp.error = L"WinHttpReceiveResponse failed: " + lastErrorToString();
    WinHttpCloseHandle(hRequest); WinHttpCloseHandle(hConnect); WinHttpCloseHandle(hSession);
    return resp;
  }
