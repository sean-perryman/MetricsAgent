...
static void setState(DWORD state){ gStatus.dwCurrentState=state; SetServiceStatus(gHandle, &gStatus); }

static void WINAPI ctrlHandler(DWORD code){
  if(code==SERVICE_CONTROL_STOP){ setState(SERVICE_STOP_PENDING); if(gStopEvent) SetEvent(gStopEvent); }
}
...
static void WINAPI serviceMain(DWORD, LPWSTR*){
  ...
  if(!loadConfigFromFile(gCfgPath, cfg, err)){ setState(SERVICE_STOPPED); return; }
...
int runService(const std::wstring& configPath){
  gCfgPath=configPath;
  SERVICE_TABLE_ENTRYW table[]={{(LPWSTR)L"MetricsAgent",(LPSERVICE_MAIN_FUNCTIONW)serviceMain},{nullptr,nullptr}};
  if(!StartServiceCtrlDispatcherW(table)) return runConsole(configPath);
  return 0;
}
